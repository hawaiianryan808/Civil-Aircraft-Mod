...
--[[
	...
	* Note: MOIs cannot be negative.
	...
	Note: 	the POI can be positive OR NEGATIVE! The other POI crossproducts are typically
			zero for an aircraft symmetric across the longitudinal-vertical plane and DCS
	...
	3. Intuition check:
		The sign of the product of inertia is determined by the "tilt" of the aircraft's
		Principal Axis of Inertia relative to its Body Axis (Reference Line).
		
		Most modern transport aircraft have heavy components (engines, wing structure,
		landing gear) BELOW the CG.
		
		This lowers the mass centroid in the middle but also leaves the tail high. These
		factors combined causes the Principal Axis of Inertia to be tilted nose-down
		relative to the fuselage waterline/longitudinal axis.
		
		In other words, absent any external forces/torque, if you could "hang" the plane
		by a rod traveling through the CG parallel to the line connecting opposing
		wingtips then the plane would tend to want to rotate to hang nose-down on its own.
		
		
		Under Western notation/convention, this nose-down tilt results in a POSITIVE I_xz.
		
		Under TsAGI notation/convention, this nose-down tilt results in a NEGATIVE I_xy.
...
--[[
	...
	center_of_mass = {-23.5 + nose_x, -0.5 + nose_y, 0.0},	-- [m] CG w.r.t. EDM 3D mesh origin in TsAGI coordinate order

--[[
	B757-300 MOI and POI (EMPTY configuation) in TsAGI coordinate system.
    Derived from 757-200 aerodynamic data and scaled for the +7.1m fuselage extension
	and associated mass increase.
    Note: 757-300 is a "flying pencil," so Yaw/Pitch inertia is high relative to Roll.
	
	POI sign check:
	The 757 has two heavy, podded engines located below the wing (-Y in TsAGI) and
    ...
    -- Note: 757-300 is a "flying pencil," so Yaw/Pitch inertia is high relative to Roll.
	-- moment_of_inertia	= {3.85e6, 19.8e6, 22.95e6, 410e3},	-- [kg*m^2] {Roll, Yaw, Pitch, Product_of_inertia(I_xz in USA notation}
    ...
    -- Limits
    Ny_min      	= -1.52,    -- [G] Minimum normal G-load
    Ny_max      	= 2.5,      -- [G] Maximum normal G-load
    Ny_max_e    	= 2.5,      -- [G] Emergency G-load
    bank_angle_max  = 35,       -- [degrees] Maximum bank angle

    -- Takeoff Geometry
    AOA_take_off    = math.rad(7.0),    -- [rad] Takeoff rotation angle
    ...
    IR_emission_coeff           = 1.6,	-- High thrust-to-weight twins. Significant mass flow creates a large "volume" of IR energy even if cooler.
